...
def _build_pdf(ws, hdr_meno, hdr_sap, hdr_ucet, hdr_spol, logo_bytes: Optional[bytes], header_hex="#25B3AD"):
    ...
    # HLAVIČKA: [logo] 10pt [textový stĺpec]
    ...
    # bold labely v meta riadku
    meta_style = ParagraphStyle("Meta", parent=styles["Base"], fontName=FONT_REG, fontSize=9, leading=12)
    meta_text = (
        f"{hdr_spol} — "
    ...
    text_block = [title, date_p, meta]

    header_tbl_data = []
    if logo_bytes:
        rlimg = RLImage(BytesIO(logo_bytes), width=60, height=60)
        header_tbl_data.append([rlimg, "", text_block])
        col_head_widths = [60, 10, None]
    else:
        header_tbl_data.append(["", "", text_block])
    ...
    header_tbl.setStyle(TableStyle([
        ...
        ("BOTTOMPADDING", (0,0), (-1,-1), 6),   # kompaktnejšie (bolo 8)
    ]))

    story = [header_tbl, Spacer(1, 6)]         # menší odstup (bolo 8)
    ...
    table.setStyle(TableStyle([
        ...
        # mriežka a zebra (jemnejšia mriežka #E2E8F0)
        ...
        # posledný riadok – rovnaká farba ako header + biele písmo + bold len na „Súčet“ a hodnotu
        ("BACKGROUND", (0,-1), (-1,-1), colors.HexColor(header_hex)),
